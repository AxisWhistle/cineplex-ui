...
//		
		if(this.isManager()){
			...
			for(Movie m:movieMngList){
				MovieInfo movieInfo =new MovieInfo(m);
			}
...
//			}
		...
		List<Schedule> movieRList=scheduleService.availableSchedules();
		
		List<MovieInfo> movielist = ScheduleListToMovieInfoList(movieRList);
		request.setAttribute("movielist", movielist);
		
		List<Schedule> movieFList=scheduleService.futureSchedules();
		List<MovieInfo> futurelist = ScheduleListToMovieInfoList(movieFList);
	...
	private List<MovieInfo> ScheduleListToMovieInfoList(List<Schedule> movieSList) {
...
//	
	private void makeFakeData(){
			...
			String[]titles ={"热映电影1·道士下山","爱情片呢","侏罗纪嘉年华哦"};
			String[] contents={"不要犹豫快来看啊","小情侣们还等什么","冒险刺激停不下来"};
//			String[] targets={"dayMovieSchedule?movieId=0"};
			for(int i = 0;i<3;i++){
				SliderInfo sliderInfo=new SliderInfo(i,"movieCover/s"+i+".jpg" , titles[i], contents[i], "dayMovieSchedule_"+(i+1));
				sliderInfos.add(sliderInfo);
			}
			request.setAttribute("sliderlist", sliderInfos);
