...
<body class="no-trans">
					...
					<div class="col-md-3">
						<div class="form-inline">
							<ul style="list-style: none;">
								<li><img alt="loading"
									src="${sessionScope.schedule.movie.cover}"></li>
								<li><label>电影名称:</label> <span>${sessionScope.schedule.movie.name}</span></li>
								<li><label>放映厅: </label> <span>${sessionScope.schedule.hall.name}</span></li>
								<li><label>放映时间:</label> <span><s:date
											name="#session.schedule.time.time" format="yyyy-MM-dd hh:mm" /></span>~<span><s:date
											name="#session.schedule.end.time" format="yyyy-MM-dd hh:mm" /></span></li>
								<li><label>单价: </label> <span>${sessionScope.schedule.price}</span></li>
							</ul>
						</div>
					</div>
					<!-- movie info end -->
					...
					<div class="col-md-9">
						...
						<form action="ajax/buyTicket" method="post" id="buyForm">
							...
							<div class="seatlist clear">
								<div class="form-inline">
									<div class=form-group>
										<label for="price">总价</label> <input type="text" name="price"
											class="form-control" id="price" value="0" disabled>
									...
									<s:if test="#session.user.gid=='2'">
										<div class=form-group>
											<label> 折后价</label> <span class="discount">0</span>
										</div>


										<div class=form-group>
											<label>卡余额</label> <span>${sessionScope.user.member.balance}</span>
										</div>
									</s:if>
										...
										<div class="col-sm-4">
											...
											<div class="radio">
												<label> <input type="radio" class="gender"
													name="cash" value="1" checked /> 现金
												</label>
											</div>
											<div class="radio">
												<label> <input type="radio" class="gender"
													name="cash" value="0" /> 会员卡
												</label>
											</div>
	...
	<s:if test="#session.user.gid=='2'">
		<input type="hidden"
			value="${sessionScope.user.member.disaccount.disaccount}" id="dis" />
	</s:if>
	<s:else>
		<input type="hidden" value="1" id="dis" />
	</s:else>
						...
						function() {
							...
							$('.seatbox')
									.on(
											'ifChecked',
											function(event) {
												var seatinfo = this.id
														.split("-");
												var seatdetail = seatinfo[0]
														+ "排" + seatinfo[1]
														+ "座";
												callbacks_list
														.append('<li class="help-block seat'+this.id+'"><span class="seat">'
																+ seatdetail
																+ '</span>已锁定</li>');
												price += $
												{
													sessionScope.schedule.price
												}
												;
